...
def display_color_legend():    
    ...
    Pavement     = np.array([60,40,222])/256
    Tree         = np.array([128,128,0])/256
    SignSymbol   = np.array([192,128,128])/256
    Fence        = np.array([64,64,128])/256
    Car          = np.array([64,0,128])/256
    Pedestrian   = np.array([64,64,0])/256
    Bicyclist    = np.array([0,128,192])/256
    Unlabelled   = np.array([0,0,0])/256
...
def draw_plots(images, labels, predicted_labels):
    ...
    fig, axes = plt.subplots(nrows=num_images, ncols=3, figsize=(20,num_images*4))
